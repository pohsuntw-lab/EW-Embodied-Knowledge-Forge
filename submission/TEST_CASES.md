...
All cases require no account, external server, or private fixture. The generated `.ewforge` file is the observable artifact.
...
- Expected result: One validated incremental `.ewforge` file plus a receipt with note count, proposed root, and not-yet-imported disclosure.
...
- Expected result: One validated `.ewforge` file with source references and no binary executable content.
...
- Expected result: An incremental package containing only new or changed notes and a relationship receipt.
...
- Expected result: One validated package with linked translation pairs.
...
- Expected result: One validated incremental package and a receipt describing the replacement lineage.
...
- Expected behavior: Creates a package if otherwise appropriate but clearly states it has not accessed or imported into the phone; provides the companion-import step.
