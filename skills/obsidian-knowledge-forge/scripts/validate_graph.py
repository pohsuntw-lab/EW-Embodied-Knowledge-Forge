...
def checkpoint_paths(checkpoint: Path | None) -> set[str]:
...
def resolve_link(root: Path, target: str, known_paths: set[str]) -> bool:
    target = target.strip()
    ...
    if normalized in known_paths or normalized + ".md" in known_paths:
        return True
    name = Path(target).name
    return any(root.rglob(name)) or any(root.rglob(name + ".md"))


def validate(root: Path, known_paths: set[str] | None = None) -> dict:
    errors: list[dict] = []
    warnings: list[dict] = []
    ...
    for note in notes:
        ...
        for target in WIKILINK.findall(text):
            if not resolve_link(root, target, known_paths):
...
def main() -> int:
    ...
    parser.add_argument("--checkpoint", type=Path, help="Allow links to note paths recorded in a prior checkpoint")
    ...
    known = checkpoint_paths(args.checkpoint) if args.checkpoint else set()
    result = validate(args.vault, known)
